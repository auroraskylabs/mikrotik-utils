/ip firewall address-list remove [find list="TM-Block"]
/ip firewall address-list
add address=77.83.59.0/24 list=TM-Block
add address=95.85.96.0/19 list=TM-Block
add address=103.220.0.0/22 list=TM-Block
add address=119.235.112.0/20 list=TM-Block
add address=177.93.143.0/24 list=TM-Block
add address=185.69.184.0/22 list=TM-Block
add address=185.246.72.0/22 list=TM-Block
add address=194.117.52.192/26 list=TM-Block
add address=216.250.8.0/21 list=TM-Block
add address=217.65.78.0/24 list=TM-Block
add address=217.174.224.0/20 list=TM-Block

/ip firewall raw
 :if ([print count-only where src-address-list="TM-Block"] = "0") do={ add chain=prerouting action=drop src-address-list=TM-Block comment="Block TM traffic" }
