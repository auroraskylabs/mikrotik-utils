/ip firewall address-list remove [find list="GI-Block"]
/ip firewall address-list
add address=2.58.8.0/22 list=GI-Block
add address=5.150.152.0/22 list=GI-Block
add address=31.222.48.0/20 list=GI-Block
add address=45.88.36.0/22 list=GI-Block
add address=45.89.212.0/22 list=GI-Block
add address=45.94.8.0/22 list=GI-Block
add address=62.192.155.0/24 list=GI-Block
add address=62.205.128.0/19 list=GI-Block
add address=80.92.200.0/22 list=GI-Block
add address=85.115.128.0/19 list=GI-Block
add address=85.159.120.0/21 list=GI-Block
add address=85.208.60.0/22 list=GI-Block
add address=91.109.248.0/21 list=GI-Block
add address=91.198.133.0/24 list=GI-Block
add address=91.235.119.0/24 list=GI-Block
add address=92.249.5.0/24 list=GI-Block
add address=93.191.192.0/21 list=GI-Block
add address=94.26.93.0/24 list=GI-Block
add address=94.26.95.0/24 list=GI-Block
add address=94.26.97.0/24 list=GI-Block
add address=94.26.98.0/23 list=GI-Block
add address=94.26.101.0/24 list=GI-Block
add address=94.26.103.0/24 list=GI-Block
add address=94.26.109.0/24 list=GI-Block
add address=94.131.0.0/23 list=GI-Block
add address=94.131.4.0/22 list=GI-Block
add address=94.131.21.0/24 list=GI-Block
add address=94.131.22.0/24 list=GI-Block
add address=94.131.24.0/21 list=GI-Block
add address=94.131.32.0/19 list=GI-Block
add address=94.131.64.0/20 list=GI-Block
add address=94.131.124.0/22 list=GI-Block
add address=94.131.128.0/19 list=GI-Block
add address=94.131.160.0/20 list=GI-Block
add address=94.131.224.0/20 list=GI-Block
add address=94.190.196.0/22 list=GI-Block
add address=95.131.184.0/21 list=GI-Block
add address=95.164.0.0/16 list=GI-Block
add address=104.255.128.0/21 list=GI-Block
add address=147.189.164.0/22 list=GI-Block
add address=153.94.56.0/21 list=GI-Block
add address=178.170.238.0/23 list=GI-Block
add address=178.170.240.0/23 list=GI-Block
add address=178.208.192.0/19 list=GI-Block
add address=178.211.128.0/24 list=GI-Block
add address=185.9.212.0/22 list=GI-Block
add address=185.16.192.0/22 list=GI-Block
add address=185.39.28.0/23 list=GI-Block
add address=185.49.216.0/22 list=GI-Block
add address=185.74.72.0/22 list=GI-Block
add address=185.75.80.0/22 list=GI-Block
add address=185.82.48.0/22 list=GI-Block
add address=185.83.148.0/22 list=GI-Block
add address=185.94.76.0/22 list=GI-Block
add address=185.104.220.0/22 list=GI-Block
add address=185.114.228.0/22 list=GI-Block
add address=185.120.96.0/22 list=GI-Block
add address=185.149.68.0/22 list=GI-Block
add address=185.192.4.0/22 list=GI-Block
add address=192.190.168.0/22 list=GI-Block
add address=193.33.64.0/23 list=GI-Block
add address=193.34.188.0/23 list=GI-Block
add address=193.105.57.0/24 list=GI-Block
add address=193.169.30.0/24 list=GI-Block
add address=194.164.146.0/23 list=GI-Block
add address=195.149.96.0/24 list=GI-Block
add address=195.166.192.0/19 list=GI-Block
add address=195.214.212.0/22 list=GI-Block
add address=195.244.192.0/19 list=GI-Block
add address=212.22.224.0/19 list=GI-Block
add address=212.86.97.0/24 list=GI-Block
add address=212.86.103.0/24 list=GI-Block
add address=212.86.107.0/24 list=GI-Block
add address=212.86.110.0/23 list=GI-Block
add address=212.86.112.0/23 list=GI-Block
add address=212.86.116.0/22 list=GI-Block
add address=212.86.120.0/21 list=GI-Block
add address=212.120.224.0/19 list=GI-Block
add address=217.65.48.0/20 list=GI-Block
add address=217.147.112.0/20 list=GI-Block

/ip firewall raw
 :if ([print count-only where src-address-list="GI-Block"] = "0") do={ add chain=prerouting action=drop src-address-list=GI-Block comment="Block GI traffic" }
