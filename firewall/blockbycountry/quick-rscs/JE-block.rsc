/ip firewall address-list remove [find list="JE-Block"]
/ip firewall address-list
add address=5.35.160.0/21 list=JE-Block
add address=5.42.128.0/21 list=JE-Block
add address=31.186.112.0/21 list=JE-Block
add address=37.156.38.0/23 list=JE-Block
add address=46.254.248.0/21 list=JE-Block
add address=81.20.176.0/20 list=JE-Block
add address=82.112.128.0/19 list=JE-Block
add address=83.137.248.0/21 list=JE-Block
add address=87.237.128.0/21 list=JE-Block
add address=87.244.64.0/18 list=JE-Block
add address=93.191.200.0/21 list=JE-Block
add address=109.68.192.0/22 list=JE-Block
add address=146.19.127.0/24 list=JE-Block
add address=165.250.0.0/16 list=JE-Block
add address=173.255.144.0/21 list=JE-Block
add address=185.3.52.0/22 list=JE-Block
add address=185.16.68.0/22 list=JE-Block
add address=185.48.60.0/22 list=JE-Block
add address=185.57.212.0/22 list=JE-Block
add address=185.87.8.0/22 list=JE-Block
add address=185.113.12.0/22 list=JE-Block
add address=185.122.244.0/22 list=JE-Block
add address=185.158.120.0/22 list=JE-Block
add address=185.179.89.0/24 list=JE-Block
add address=185.179.101.0/24 list=JE-Block
add address=185.206.12.0/22 list=JE-Block
add address=185.235.244.0/24 list=JE-Block
add address=188.116.44.0/24 list=JE-Block
add address=193.3.239.0/24 list=JE-Block
add address=193.17.36.0/22 list=JE-Block
add address=193.36.44.0/24 list=JE-Block
add address=193.201.42.0/24 list=JE-Block
add address=194.34.106.0/23 list=JE-Block
add address=199.34.116.0/22 list=JE-Block
add address=199.66.128.0/22 list=JE-Block
add address=209.251.252.0/23 list=JE-Block
add address=212.9.0.0/19 list=JE-Block
add address=213.133.192.0/19 list=JE-Block
add address=213.134.23.0/24 list=JE-Block
add address=217.198.188.0/24 list=JE-Block

/ip firewall raw
 :if ([print count-only where src-address-list="JE-Block"] = "0") do={ add chain=prerouting action=drop src-address-list=JE-Block comment="Block JE traffic" }
